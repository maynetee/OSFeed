...
import { MessageSquare, Filter, Activity, Plus, FileText, FolderOpen, Radio } from 'lucide-react'
...
const PatternDetailModal = lazy(() =>
  import('@/components/analysis/pattern-detail-modal').then((m) => ({
    default: m.PatternDetailModal,
  })),
)
const EscalationChart = lazy(() => import('@/components/stats/escalation-chart'))
...
export function DashboardPage() {
  ...
  const escalationQuery = useQuery({
    ...
    queryFn: async () => {
      const collectionId = selectedCollection === 'all' ? undefined : selectedCollection
      return (await analysisApi.getEscalationTrend({ collection_id: collectionId, period: '7d' }))
        .data
    },
    refetchInterval: 300000,
  })
  ...
  const kpiDuplicates =
    selectedCollection === 'all'
      ? Math.round(
          ((dashboardData?.overview?.duplicates_last_24h ?? 0) /
            ((dashboardData?.overview?.messages_last_24h ?? 0) || 1)) *
            100,
        )
    ...
    <PageTransition>
      ...
      >
        ...
        <motion.div
          ...
          className="rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/5 via-transparent to-primary/5 p-8"
        >
          <h1 className="text-3xl font-bold tracking-tight">
            {t('dashboard.welcome')}
          </h1>
          <p className="mt-2 text-foreground/60">{t('dashboard.subtitle')}</p>
        </motion.div>
        ...
        <motion.div variants={staggerItem} className="flex flex-wrap gap-3">
          <Button
            ...
            size="sm"
            className="gap-2"
            onClick={() => navigate('/channels?add=true')}
          >
            <Plus className="h-4 w-4" />
            {t('dashboard.quickAddChannel')}
          </Button>
          <Button
            variant="outline"
            size="sm"
            ...
            <CardContent className="py-6">
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="flex items-center gap-6 text-sm text-muted-foreground">
                  <span>
                    {t('dashboard.signalsToday')}:{' '}
                    <strong className="text-foreground">
                      {(insights?.signals_processed?.today ?? 0).toLocaleString()}
                    </strong>
                  </span>
                  <span>
                    {t('dashboard.signalsThisWeek')}:{' '}
                    <strong className="text-foreground">
                      {(insights?.signals_processed?.this_week ?? 0).toLocaleString()}
                    </strong>
                  </span>
                </div>
              </div>
        ...
        {/* KPI Cards */}
        <motion.section variants={staggerItem} className="grid gap-4 md:grid-cols-3">
          <KpiCard
        ...
        <motion.section variants={staggerItem} className="grid gap-6 lg:grid-cols-3">
          {/* Intelligence Tips */}
          <Card>
            ...
            <CardContent className="space-y-3">
              {tips.length === 0 ? (
                ...
                tips.map((tip, i) => (
                  <div
                    key={i}
                    className={`rounded-lg border p-3 ${SEVERITY_COLORS[tip.severity] || SEVERITY_COLORS.info}`}
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`h-2 w-2 rounded-full ${SEVERITY_DOT[tip.severity] || SEVERITY_DOT.info}`}
                      />
          ...
          <Card>
            ...
            <CardContent>
              {!escalationTrend || escalationTrend.trend.length === 0 ? (
                ...
                <Suspense fallback={<div className="h-48 w-full animate-pulse rounded bg-muted" />}>
                  <EscalationChart data={escalationTrend.trend} />
                </Suspense>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">
                {t('analysis.dashboard.detectedPatterns')}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
                ...
                detectedPatterns.map((pattern) => (
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <Badge
                        className={
                          PATTERN_TYPE_BADGE[pattern.pattern_type] ?? 'bg-muted text-foreground/60'
                        }
                      >
        ...
        {/* Activity & Channel Ranking */}
        <motion.section variants={staggerItem} className="grid gap-6 lg:grid-cols-[2fr_1fr]">
          <Card>
            <CardHeader>
