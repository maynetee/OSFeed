        ...
        <Card>
          <CardContent className="py-4">
            ...
            <p className="text-sm font-medium">
              {alert.last_triggered_at ? <Timestamp value={alert.last_triggered_at} /> : t('alerts.never')}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="py-4">
            ...
            <div className="flex flex-wrap gap-1 mt-1">
              {(alert.keywords ?? []).map((kw) => (
                <span key={kw} className="inline-flex items-center rounded-full bg-primary/10 px-2 py-0.5 text-xs text-primary">{kw}</span>
              ))}
              {(!alert.keywords || alert.keywords.length === 0) && <span className="text-xs text-foreground/40">&mdash;</span>}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div>
        ...
        {triggersQuery.isLoading ? (
          <div className="space-y-2">
            {[1, 2, 3].map((i) => (
              <Card key={i}><CardContent className="h-16 animate-pulse py-4" /></Card>
            ))}
          </div>
        ) : triggers.length === 0 ? (
          <Card>
            ...
            {triggers.map((trigger) => (
              <Card key={trigger.id}>
                <CardContent className="flex items-center justify-between py-3">
                  <div>
                    <p className="text-sm font-medium">{trigger.summary ?? t('alerts.triggered')}</p>
                    <p className="text-xs text-foreground/50">{trigger.message_ids.length} {t('alerts.matchedMessages')}</p>
                  </div>
                  <p className="text-xs text-foreground/40"><Timestamp value={trigger.triggered_at} /></p>
                </CardContent>
              </Card>
            ))}
          </div>
