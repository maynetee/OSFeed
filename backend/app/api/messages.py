...
async def get_message_media(
    ...
    try:
        ...
        logger.error(f"Unexpected error fetching media for message {message_id}: {type(e).__name__}: {e}", exc_info=True)
        raise HTTPException(status_code=502, detail="MEDIA_FETCH_ERROR")
